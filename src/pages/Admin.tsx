...
// --- Reusable Multi-Select Text Input with Suggestions ---
...
const MultiSelectInput: React.FC<MultiSelectInputProps> = ({ label, values, options, onChange, onAddOption, placeholder, required }) => {
    ...
    // Sync input value with parent values
    useEffect(() => {
    ...
    const handleSelectSuggestion = (suggestion: string) => {
        ...
        currentSegments.pop(); // Remove the partial segment being typed
...
const Admin: React.FC = () => {
  ...
  // --- Master Lists State (The Central Labels Database) ---
  ...
  // --- Library State ---
  const [documentsList, setDocumentsList] = useState<DocumentItem[]>([]); // Initialized via useEffect
  const [isUploadingDoc, setIsUploadingDoc] = useState(false);
  const [currentDoc, setCurrentDoc] = useState<Partial<DocumentItem>>({});

  // Load documents, catalysts, and events from local storage on mount
  useEffect(() => {
    ...
    if (savedCatalysts) setCatalysts(JSON.parse(savedCatalysts));
        ...
        // Fallback default for first run if needed
        setCatalysts([ 
            { 
                id: '1', 
  ...
  const [tickerDateFilter, setTickerDateFilter] = useState('');
      ...
      const headers = [
          {
              ...
              'Summary': 'Detailed summary of the intelligence event...',
              'CI Note': 'Internal strategic implications...',
              'Companies': 'Novartis, Bayer',
              'Assets': 'Pluvicto',
              'Target': 'PSMA',
              'Tumor Type': 'Prostate Cancer',
              'Isotope': '177Lu',
              'Isotope Type': 'Beta',
              'Asset Focus': 'Therapeutic',
              'Region': 'Global',
              'Phase': 'Phase 3',
              'Line of Therapy': '1L',
              'Source Name': 'Company Press Release',
      ...
      reader.onload = (evt) => {
          ...
          setTimeout(() => {
              try {
                  ...
                  // Use cellDates: true to handle Excel date serials correctly
                  const wb = XLSX.read(bstr, { type: 'binary', cellDates: true });
                  setUploadProgress(60); // Parsed
                  const wsname = wb.SheetNames[0];
                  const ws = wb.Sheets[wsname];
                  const data = XLSX.utils.sheet_to_json(ws);
                  setUploadProgress(80); // Converting
                  ...
                  const newNewsItems: NewsItem[] = data.map((row: any, index: number) => {
                      ...
                      if (row['Date']) {
                          ...
                          if (!isNaN(dateObj.getTime())) {
                              // Use local time components to preserve the calendar date from Excel
                              // This avoids timezone shifts (e.g. 00:00 midnight becoming previous day in UTC)
  ...
  const openAddToGroupModal = (user: UserData) => {
      setUserForGroupAddition(user);
  ...
  const handleSaveDoc = (e: React.FormEvent) => {
      ...
      if (!docToSave.id) {
          ...
          // LocalStorage update (if applicable)
          const savedDocs = localStorage.getItem('uploadedDocuments');
          if (savedDocs) {
  ...
  // ... (Ticker/Home Handlers remain same) ...
  const handleAddToTicker = (newsItem: NewsItem) => {
      const newItem = {
          id: Date.now(),
          type: 'BREAKING',
          text: newsItem.title,
      ...
      setNewsList(prev => prev.map(n => n.id === newsItem.id ? { ...n, isBreaking: true } : n));
      setIsTickerModalOpen(false);
      showNotification('News added to ticker');
  };
  ...
  const handlePushTickerUpdates = () => {
      showNotification('Live ticker updated globally');
  };
  ...
  const filteredNews = newsList.filter(item => {
    ...
    const matchesTab = newsSubTab === 'all' 
                       ? true 
                       : !item.isHumanReviewed; 
    return matchesSearch && matchesTab;
  });

  const tickerModalNews = newsList.filter(item => {
      const matchesSearch = item.title.toLowerCase().includes(tickerSearch.toLowerCase());
      const matchesDate = tickerDateFilter ? item.date === tickerDateFilter : true;
      return matchesSearch && matchesDate;
  });

  // Label Categories Config for Sidebar
  ...
  return (
    <div className="min-h-screen pt-24 px-8 pb-8 font-sans bg-gray-50/50">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex gap-6 border-b border-gray-200 mb-6 overflow-x-auto">
            <button 
                ...
                className={`pb-3 px-2 text-sm font-medium flex items-center gap-2 transition-all whitespace-nowrap ${activeTab === 'news' ? 'border-b-2 border-brand-cyan text-brand-cyan' : 'text-gray-500 hover:text-gray-800'}`}
            >
                <Database size={16} /> News Database
            </button>
            <button 
                onClick={() => setActiveTab('events')}
                className={`pb-3 px-2 text-sm font-medium flex items-center gap-2 transition-all whitespace-nowrap ${activeTab === 'events' ? 'border-b-2 border-brand-cyan text-brand-cyan' : 'text-gray-500 hover:text-gray-800'}`}
            >
                <Calendar size={16} /> Catalysts & Events
            </button>
            <button 
                onClick={() => setActiveTab('library')}
                className={`pb-3 px-2 text-sm font-medium flex items-center gap-2 transition-all whitespace-nowrap ${activeTab === 'library' ? 'border-b-2 border-brand-cyan text-brand-cyan' : 'text-gray-500 hover:text-gray-800'}`}
            >
                <BookOpen size={16} /> Library
            </button>
            <button 
                onClick={() => setActiveTab('labels')}
                className={`pb-3 px-2 text-sm font-medium flex items-center gap-2 transition-all whitespace-nowrap ${activeTab === 'labels' ? 'border-b-2 border-brand-cyan text-brand-cyan' : 'text-gray-500 hover:text-gray-800'}`}
            >
                <Tag size={16} /> Labels
            </button>
            <button 
                onClick={() => setActiveTab('users')}
                className={`pb-3 px-2 text-sm font-medium flex items-center gap-2 transition-all whitespace-nowrap ${activeTab === 'users' ? 'border-b-2 border-brand-cyan text-brand-cyan' : 'text-gray-500 hover:text-gray-800'}`}
            >
                <Users size={16} /> User Management
            </button>
            <button 
                onClick={() => setActiveTab('portal')}
                className={`pb-3 px-2 text-sm font-medium flex items-center gap-2 transition-all whitespace-nowrap ${activeTab === 'portal' ? 'border-b-2 border-brand-cyan text-brand-cyan' : 'text-gray-500 hover:text-gray-800'}`}
            >
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 flex flex-col h-full">
                        <div className="flex items-center gap-3 mb-6 pb-4 border-b border-gray-100">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-gray-500">Manage high-impact upcoming milestones.</p>
                            </div>
                        </div>
                        ...
                        <div className="mb-8 bg-gray-50 rounded-xl p-4 border border-gray-200">
                            ...
                            <div className="space-y-3">
                                <input 
                                    ...
                                    value={newCatalyst.title || ''}
                                    onChange={(e) => setNewCatalyst({...newCatalyst, title: e.target.value})}
                                    className="w-full p-2 border border-gray-200 rounded-lg text-sm outline-none focus:border-brand-cyan"
                                />
                                <div className="grid grid-cols-2 gap-3">
                                    <input 
                                        type="text" 
                                        placeholder="Date (e.g. Jun 15)" 
                                        value={newCatalyst.date || ''}
                                        onChange={(e) => setNewCatalyst({...newCatalyst, date: e.target.value})}
                                        className="w-full p-2 border border-gray-200 rounded-lg text-sm outline-none focus:border-brand-cyan"
                                    />
                                    <input 
                                        type="text" 
                                        placeholder="Or Quarter (e.g. Q3 2025)"
                                        value={newCatalyst.quarter || ''}
                                        onChange={(e) => setNewCatalyst({...newCatalyst, quarter: e.target.value})}
                                        className="w-full p-2 border border-gray-200 rounded-lg text-sm outline-none focus:border-brand-cyan"
                                    />
                                </div>
                                <div className="flex gap-3">
                                    <input 
                                        ...
                                        value={newCatalyst.url || ''}
                                        onChange={(e) => setNewCatalyst({...newCatalyst, url: e.target.value})}
                                        className="flex-1 p-2 border border-gray-200 rounded-lg text-sm outline-none focus:border-brand-cyan"
                                    />
                                    <button 
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar space-y-2">
                                    ...
                                    <div className="flex items-start gap-3">
                                        ...
                                        <div>
                                            ...
                                            <div className="flex items-center gap-2 text-xs text-gray-500">
                                                <span>{cat.date || cat.quarter}</span>
                                                {cat.url && <a href={cat.url} target="_blank" rel="noreferrer" className="text-brand-cyan hover:underline flex items-center gap-0.5"><LinkIcon size={10} /> Link</a>}
                                            </div>
                                        </div>
                                    </div>
                                    <button 
                            ...
                            {catalysts.length === 0 && <p className="text-center text-gray-400 text-sm py-4">No catalysts added.</p>}
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 flex flex-col h-full">
                        <div className="flex items-center gap-3 mb-6 pb-4 border-b border-gray-100">
                            ...
                            <div>
                                <h2 className="text-lg font-bold text-gray-900">Events Calendar</h2>
                                <p className="text-xs text-gray-500">Manage conferences and key meetings.</p>
                            </div>
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar space-y-2">
                            ...
                            {events.length === 0 && <p className="text-center text-gray-400 text-sm py-4">No events added.</p>}
                        </div>
                    </div>
                </div>
            )}
                ...
                {isUploadingDoc ? (
                    ...
                    <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-8">
                        <div className="flex justify-between items-center mb-6 pb-6 border-b border-gray-100">
                            <h2 className="text-xl font-bold text-gray-900 flex items-center gap-2">
                                <Upload size={20} className="text-brand-cyan" /> {currentDoc.id ? 'Edit Document' : 'Upload New Document'}
                            </h2>
                        ...
                        <form onSubmit={handleSaveDoc} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                            {/* Column 1: File & Basic Info */}
                            <div className="space-y-6 lg:col-span-2">
                                <CreatableSelect 
                                    label="Upload By (Team)"
                                    value={currentDoc.uploadedBy || ''}
                                    options={options.uploadTeams}
                                    onChange={(val) => setCurrentDoc({...currentDoc, uploadedBy: val})}
                                    onAddOption={(val) => addOption('uploadTeams', val)}
                                ...
                                <div 
                                    ...
                                    className="border-2 border-dashed border-gray-200 rounded-xl p-8 flex flex-col items-center justify-center text-center hover:border-brand-cyan hover:bg-brand-cyan/5 transition-all cursor-pointer group"
                                >
                                    <input 
                                        type="file" 
                                        ref={fileInputRef} 
                                    ...
                                    {currentDoc.fileSize && currentDoc.fileSize !== '0 MB' ? (
                                        <div className="animate-[fadeIn_0.3s_ease-out]">
                                            ...
                                            <p className="text-xs text-gray-500 mt-1">{currentDoc.fileSize} • {currentDoc.format}</p>
                                        </div>
                                    ) : (
                                        <>
                                            <div className="w-12 h-12 bg-gray-100 rounded-full flex items-center justify-center mb-3 group-hover:bg-brand-cyan/20 group-hover:text-brand-cyan transition-colors">
                                                <File size={24} className="text-gray-400 group-hover:text-brand-cyan" />
                                            </div>
                                            <p className="text-sm font-bold text-gray-900">Click to upload file</p>
                                            <p className="text-xs text-gray-500 mt-1">PDF, PPT, DOCX, XLSX</p>
                                        </>
                                    )}
                                </div>
                                ...
                                <div>
                                    ...
                                    <input 
                                        ...
                                        className="w-full p-2 border border-gray-200 rounded-lg text-sm bg-white text-gray-900"
                                        placeholder="e.g. Q3 Competitive Landscape Report"
                                        required
                                    />
                                </div>
                                ...
                                <div>
                                    ...
                                    <AutoResizeTextarea 
                                        value={currentDoc.executiveSummary || ''} 
                                        onChange={e => setCurrentDoc({...currentDoc, executiveSummary: e.target.value})}
                                        className="w-full p-2 border border-gray-200 rounded-lg text-sm outline-none min-h-[8rem] bg-white text-gray-900"
                            ...
                            <div className="space-y-4">
                                ...
                                <div>
                                    ...
                                    <input 
                                        ...
                                        className="w-full p-2 border border-gray-200 rounded-lg text-sm bg-white text-gray-900 outline-none focus:border-brand-cyan"
                                    />
                                </div>

                                <CreatableSelect 
                                    label="User Group Access"
                                    value={currentDoc.userGroup || ''}
                                    options={options.userGroups}
                                    onChange={(val) => setCurrentDoc({...currentDoc, userGroup: val})}
                                    onAddOption={(val) => addOption('userGroups', val)}
                                ...
                                <CreatableSelect 
                                    ...
                                    options={options.docCategories}
                                    onChange={(val) => setCurrentDoc({...currentDoc, type: val})}
                                    onAddOption={(val) => addOption('docCategories', val)}
                                    placeholder="Select..."
                                    required
                                />
                                ...
                                <MultiSelectInput 
                                    label="Authors"
                                    values={currentDoc.author || []}
                                    options={combinedAuthorOptions}
                                    onChange={(vals) => setCurrentDoc({...currentDoc, author: vals})}
                                    onAddOption={(val) => addOption('authors', val)}
                                    placeholder="Add authors..."
                                />

                                <MultiSelectInput 
                                    label="Tags"
                                    values={currentDoc.tags || []}
                                    options={options.tags}
                                    onChange={(vals) => setCurrentDoc({...currentDoc, tags: vals})}
                                    onAddOption={(val) => addOption('tags', val)}
                            ...
                            <div className="col-span-1 lg:col-span-3 pt-6 flex justify-end gap-3 border-t border-gray-100 mt-4">
                                {currentDoc.id && (
                                    <button
                                        type="button"
                                        onClick={handleDeleteDocFromForm}
                                ...
                                <button 
                                    type="submit"
                                    className="px-6 py-2 text-sm font-bold text-white bg-brand-cyan hover:bg-brand-teal rounded-lg shadow-lg shadow-brand-cyan/20 transition-all flex items-center gap-2"
                                >
                    ...
                    <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
                        <div className="p-4 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
                            <div>
                                <h2 className="text-xl font-bold text-gray-900 mb-1">Document Library</h2>
                                <p className="text-sm text-gray-500">View and manage uploaded files.</p>
                            </div>
                            <button 
                                onClick={handleUploadClick}
                                className="flex items-center justify-center gap-2 px-4 py-2 bg-gray-900 text-white text-sm font-medium rounded-lg hover:bg-black transition-colors"
                            >
                            ...
                            <table className="w-full text-left border-collapse">
                                <thead>
                                    <tr className="bg-gray-50 border-b border-gray-100 text-xs font-bold text-gray-500 uppercase tracking-wider">
                                        ...
                                        <th className="px-6 py-4">Uploaded By (Team)</th>
                                        ...
                                        <tr key={doc.id} className="hover:bg-gray-50 transition-colors">
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-3">
                                                    <div className={`w-10 h-10 rounded-lg flex items-center justify-center text-xs font-bold border ${doc.format === 'PPT' ? 'bg-orange-50 border-orange-100 text-orange-700' : 'bg-red-50 border-red-100 text-red-700'}`}>
                                                        {doc.format}
                                                    </div>
                                                    <div>
                                                        <div className="text-sm font-bold text-gray-900 line-clamp-1">{doc.title}</div>
                                                        <div className="text-xs text-gray-500">{doc.fileSize} • {Array.isArray(doc.author) ? doc.author.join(', ') : doc.author}</div>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4">
                                                <span className={`inline-flex px-2 py-1 rounded text-xs font-bold ${getDocCategoryStyle(doc.type)}`}>
                                                    {doc.type}
                                                </span>
                                            </td>
                                            ...
                                            <td className="px-6 py-4 text-right">
                                                <button 
                                                    onClick={() => handleEditLibraryDoc(doc)}
                                                    className="flex items-center gap-1 text-xs font-medium text-gray-500 hover:text-brand-cyan hover:bg-gray-100 px-2 py-1 rounded transition-colors ml-auto"
                                                >
                                                    <Edit3 size={14} /> Edit
                                                </button>
                                            </td>
                ...
                <div className="grid grid-cols-12 gap-8 h-[calc(100vh-200px)]">
                    <div className="col-span-3 bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden flex flex-col">
                        <div className="p-4 border-b border-gray-100 bg-gray-50">
                            <h3 className="font-bold text-gray-900 text-sm uppercase tracking-wide">Classifications</h3>
                        </div>
                        <div className="flex-1 overflow-y-auto p-2 space-y-1">
                            {labelCategories.map(cat => (
                                <button
                                    key={cat.key}
                                    onClick={() => setActiveLabelCategory(cat.key)}
                                    className={`w-full flex items-center justify-between px-3 py-2 rounded-lg text-sm transition-colors ${
                                        activeLabelCategory === cat.key 
                         ...
                         <div className="p-6 border-b border-gray-100 flex justify-between items-center">
                             <div>
                                 <h2 className="text-xl font-bold text-gray-900 mb-1">
                                     Manage {labelCategories.find(c => c.key === activeLabelCategory)?.label}
                                 </h2>
                                 <p className="text-sm text-gray-500">Add or remove standardized labels for this category.</p>
                             </div>
                             ...
                             <div className="flex gap-2">
                                 <input 
                                     ...
                                     placeholder={`Add new ${labelCategories.find(c => c.key === activeLabelCategory)?.label.slice(0,-1)}...`}
                                     className="w-64 border border-gray-200 bg-white text-gray-900 rounded-lg px-3 py-2 text-sm outline-none focus:border-brand-cyan"
                                     onKeyDown={(e) => {
                                         if (e.key === 'Enter') {
                                             addOption(activeLabelCategory, newLabelInput);
                                             setNewLabelInput('');
                                         }
                                     }}
                                 />
                                 <button 
                                     ...
                                     className="flex items-center justify-center gap-2 px-4 py-2 bg-gray-900 text-white text-sm font-medium rounded-lg hover:bg-black transition-colors w-full md:w-auto disabled:opacity-50"
                                 >
                                     <Plus size={16} /> Add
                                 </button>
                             </div>
                             ...
                             <div className="flex flex-wrap gap-2">
                                 {options[activeLabelCategory].sort().map(item => (
                                     <div key={item} className="flex items-center gap-2 px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg group hover:border-brand-cyan/30 hover:bg-brand-cyan/5 transition-all">
                                         ...
                                         <button 
                                             onClick={() => removeOption(activeLabelCategory, item)}
                                             className="text-gray-400 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity"
                                         >
                                             <X size={14} />
                                         </button>
                    ...
                    <div className="p-6 border-b border-gray-100 flex flex-col md:flex-row justify-end items-start md:items-center gap-4 bg-gray-50/50">
                        <div className="flex items-center gap-2 bg-gray-200/50 p-1 rounded-lg mr-auto">
                            <button 
                                ...
                                className={`px-4 py-2 text-sm font-medium rounded-md transition-all ${userSubTab === 'all' ? 'bg-white text-gray-900 shadow-sm font-bold' : 'text-gray-500 hover:text-gray-900'}`}
                            >
                                All Users
                            </button>
                            <button 
                                onClick={() => setUserSubTab('pending')}
                                className={`px-4 py-2 text-sm font-medium rounded-md transition-all flex items-center gap-2 ${userSubTab === 'pending' ? 'bg-white text-gray-900 shadow-sm font-bold' : 'text-gray-500 hover:text-gray-900'}`}
                            >
                                Pending
                                {users.filter(u => u.status === 'Pending').length > 0 && (
                                    <span className="w-5 h-5 rounded-full bg-red-500 text-white text-[10px] flex items-center justify-center font-bold">{users.filter(u => u.status === 'Pending').length}</span>
                                )}
                            </button>
                             <button 
                                onClick={() => setUserSubTab('groups')}
                                className={`px-4 py-2 text-sm font-medium rounded-md transition-all ${userSubTab === 'groups' ? 'bg-white text-gray-900 shadow-sm font-bold' : 'text-gray-500 hover:text-gray-900'}`}
                            >
                                User Groups
                            </button>
                        </div>
                        <button 
                            ...
                            className="flex items-center gap-2 px-4 py-2 bg-gray-900 text-white text-sm font-bold rounded-lg hover:bg-black transition-colors shadow-md"
                        >
                            ...
                            <div className="overflow-x-auto">
                                <table className="w-full text-left">
                                    <thead className="bg-gray-50 border-b border-gray-100">
                                        <tr>
                                            <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase w-1/4">User</th>
                                            <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase">Designation</th>
                                            <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase">Department</th>
                                            <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase">Role</th>
                                            <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase">Status</th>
                                            <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase">Groups</th>
                                    ...
                                    <tbody className="divide-y divide-gray-50">
                                        {users.filter(u => u.status !== 'Pending').map(user => (
                                            <tr key={user.id} className="hover:bg-gray-50 transition-colors">
                                                <td className="px-6 py-4">
                                                    <div className="flex items-center gap-3">
                                                        <div className="w-9 h-9 rounded-full bg-gray-100 text-gray-600 flex items-center justify-center font-bold text-sm border border-gray-200">
                                                ...
                                                <td className="px-6 py-4 text-sm text-gray-700 font-medium">{user.designation}</td>
                                                <td className="px-6 py-4 text-sm text-gray-500">{user.department}</td>
                                                <td className="px-6 py-4">
                                                    <div className={`flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider ${user.role === 'Admin' ? 'text-purple-600' : 'text-blue-600'}`}>
                                                        <Shield size={14} /> {user.role}
                                                    </div>
                                                </td>
                                                <td className="px-6 py-4">
                                                    <span className={`inline-flex px-2 py-1 rounded-full text-xs font-bold ${user.status === 'Active' ? 'bg-green-100 text-green-700' : user.status === 'Pending' ? 'bg-orange-100 text-orange-700' : 'bg-gray-100 text-gray-500'}`}>
                                                        {user.status}
                                                    </span>
                                                </td>
                                                <td className="px-6 py-4">
                                                    <div className="flex flex-wrap gap-1">
                                                        {user.groups.map(g => (
                                                            <span key={g} className="px-1.5 py-0.5 bg-gray-100 border border-gray-200 rounded text-[10px] text-gray-600">{g}</span>
                                                        ))}
                                                    </div>
                                                </td>
                                                <td className="px-6 py-4 text-right">
                                                    <div className="flex items-center justify-end gap-2">
                                                        <button onClick={() => openEditUser(user)} className="p-1.5 text-gray-400 hover:text-brand-cyan hover:bg-gray-100 rounded transition-colors"><Edit3 size={14} /></button>
                                                        <button onClick={() => handleDeleteUser(user.id)} className="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded transition-colors"><Trash2 size={14} /></button>
                                                    </div>
                                                </td>
                                            </tr>
                                        ))}
                                ...
                                <table className="w-full text-left">
                                    <thead className="bg-gray-50 border-b border-gray-100">
                                        <tr>
                                            ...
                                            <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase">Department</th>
                                            <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase">Requested</th>
                                            <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase text-right">Actions</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-gray-50">
                                        {users.filter(u => u.status === 'Pending').map(user => (
                                            <tr key={user.id} className="hover:bg-gray-50 transition-colors">
                                                ...
                                                <td className="px-6 py-4 text-sm text-gray-500">{user.department}</td>
                                                <td className="px-6 py-4 text-sm text-gray-500">{user.requestedDate}</td>
                                                <td className="px-6 py-4 text-right">
                                                    <div className="flex items-center justify-end gap-2">
                                                        <button onClick={() => handleApproveUser(user)} className="px-3 py-1 bg-green-500 text-white text-xs font-bold rounded-lg hover:bg-green-600 transition-colors">Approve</button>
                                                        <button onClick={() => initiateReject(user)} className="px-3 py-1 bg-red-100 text-red-600 text-xs font-bold rounded-lg hover:bg-red-200 transition-colors">Reject</button>
                                                    </div>
                                                </td>
                                            </tr>
                                        ))}
                                        {users.filter(u => u.status === 'Pending').length === 0 && (
                                            <tr>
                                                <td colSpan={4} className="px-6 py-8 text-center text-gray-400 text-sm">No pending requests</td>
                                            </tr>
                                        )}
                                    </tbody>
                                </table>
                            ...
                            <div className="p-6">
                                <div className="flex gap-4 mb-6">
                                    <input 
                                        ...
                                        className="flex-1 p-2 border border-gray-200 rounded-lg text-sm outline-none focus:border-brand-cyan"
                                    />
                                    <button onClick={handleCreateGroup} className="px-4 py-2 bg-gray-900 text-white text-sm font-bold rounded-lg hover:bg-black transition-colors">Create Group</button>
                                </div>
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                    {options.userGroups.map(group => (
                                        <div key={group} className="p-4 border border-gray-200 rounded-xl hover:border-brand-cyan/30 hover:bg-brand-cyan/5 transition-all group relative bg-white">
                                            <div className="flex justify-between items-start mb-2">
                                                <h4 className="font-bold text-gray-900">{group}</h4>
                                                {group !== 'All Users' && (
                                                    <button onClick={() => handleDeleteGroup(group)} className="text-gray-400 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity"><X size={14} /></button>
                                                )}
                                            </div>
                                            <p className="text-xs text-gray-500 mb-4">{users.filter(u => u.groups.includes(group)).length} Members</p>
                                            <button onClick={() => openGroupModal(group)} className="w-full py-1.5 text-xs font-medium text-brand-cyan bg-white border border-brand-cyan rounded-lg hover:bg-brand-cyan hover:text-white transition-colors">Manage Members</button>
                                        </div>
                                    ))}
                                </div>
                            </div>
                        ...
                        <div>
                         ...
                         <div className="pt-8 border-t border-gray-100">
                             ...
                             <div className="flex gap-4">
                                 <button onClick={downloadTemplate} className="px-4 py-2 border border-gray-200 text-gray-700 rounded-lg text-sm font-bold hover:bg-gray-50 flex items-center gap-2">
                                     <Download size={16} /> Download Template
                                 </button>
                                 <button onClick={() => setIsBulkModalOpen(true)} className="px-4 py-2 bg-brand-cyan text-white rounded-lg text-sm font-bold hover:bg-brand-teal flex items-center gap-2 shadow-lg shadow-brand-cyan/20">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-[fadeIn_0.2s_ease-out]">
              <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6">
                  ...
                  <form onSubmit={handleInviteUser} className="space-y-4">
                      ...
                      <select name="role" className="w-full p-2 border border-gray-200 rounded-lg text-sm">
                          <option value="User">User</option>
                          <option value="Admin">Admin</option>
                      </select>
                      <div className="flex justify-end gap-3 pt-4">
                          <button type="button" onClick={() => setIsInviteModalOpen(false)} className="px-4 py-2 text-gray-500 hover:bg-gray-100 rounded-lg text-sm font-medium">Cancel</button>
                          <button type="submit" className="px-4 py-2 bg-brand-cyan text-white rounded-lg text-sm font-bold hover:bg-brand-teal">Send Invite</button>
                      </div>
                  </form>
              </div>
          </div>
      ...
      {/* 2. Edit News Modal (Using existing logic/UI but cleaner) */}
      {isEditingNews && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-[fadeIn_0.2s_ease-out]">
             <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden">
                     ...
                     <form id="newsForm" onSubmit={handleSaveNews} className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div>
                                ...
                                <input type="date" value={currentNews.date} onChange={e => setCurrentNews({...currentNews, date: e.target.value})} className="w-full p-2 border border-gray-200 rounded-lg text-sm" required />
                            </div>
                            <div>
                                <CreatableSelect label="Entry Type" value={currentNews.entryType || ''} options={options.entryTypes} onChange={v => setCurrentNews({...currentNews, entryType: v as EntryType})} onAddOption={v => addOption('entryTypes', v)} required />
                            </div>
                             <div>
                                <CreatableSelect label="News Type" value={currentNews.type || ''} options={options.types} onChange={v => setCurrentNews({...currentNews, type: v as NewsType, subType: ''})} onAddOption={v => addOption('types', v)} required />
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                             <div>
                                <CreatableSelect label="Sub-Type" value={currentNews.subType || ''} options={getSubTypesForType(currentNews.type)} onChange={v => setCurrentNews({...currentNews, subType: v})} onAddOption={v => addOption('subTypes', v)} required />
                            </div>
                            <div className="md:col-span-3">
                                <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Title *</label>
                                <input type="text" value={currentNews.title} onChange={e => setCurrentNews({...currentNews, title: e.target.value})} className="w-full p-2 border border-gray-200 rounded-lg text-sm font-bold" required />
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <textarea value={currentNews.summary} onChange={e => setCurrentNews({...currentNews, summary: e.target.value})} className="w-full p-2 border border-gray-200 rounded-lg text-sm h-32 resize-none" />
                            </div>
                            <div>
                                <label className="block text-xs font-bold text-amber-600 uppercase mb-1">CI Note (Internal)</label>
                                <textarea value={currentNews.ciNote} onChange={e => setCurrentNews({...currentNews, ciNote: e.target.value})} className="w-full p-2 border border-amber-200 bg-amber-50 rounded-lg text-sm h-32 resize-none" />
                            </div>
                        </div>
                        ...
                        <div className="p-4 bg-gray-50 rounded-xl border border-gray-200 space-y-4">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Trial ID</label>
                                    <input type="text" value={currentNews.trialId || ''} onChange={e => setCurrentNews({...currentNews, trialId: e.target.value})} className="w-full p-2 border border-gray-200 rounded-lg text-sm" placeholder="e.g. NCT12345678" />
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-end">
                            <div>
                                ...
                                <input type="text" value={currentNews.sourceName || ''} onChange={e => setCurrentNews({...currentNews, sourceName: e.target.value})} className="w-full p-2 border border-gray-200 rounded-lg text-sm" required />
                            </div>
                            <div>
                                <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Source URL *</label>
                                <input type="url" value={currentNews.sourceUrl || ''} onChange={e => setCurrentNews({...currentNews, sourceUrl: e.target.value})} className="w-full p-2 border border-gray-200 rounded-lg text-sm" required />
                            </div>
                            <div className="flex items-center gap-4">
                                <label className="flex items-center gap-2 cursor-pointer bg-gray-50 px-3 py-2 rounded-lg border border-gray-200">
                                    <input type="checkbox" checked={currentNews.priority === 'Key'} onChange={e => setCurrentNews({...currentNews, priority: e.target.checked ? 'Key' : 'Other'})} className="rounded text-brand-cyan" />
                                    <span className="text-sm font-bold text-gray-700">Key Priority</span>
                                </label>
                                <label className="flex items-center gap-2 cursor-pointer bg-red-50 px-3 py-2 rounded-lg border border-red-100">
                                    <input type="checkbox" checked={currentNews.isBreaking} onChange={e => setCurrentNews({...currentNews, isBreaking: e.target.checked})} className="rounded text-red-500" />
                                    <span className="text-sm font-bold text-red-600">Breaking News</span>
                                </label>
                            </div>
                        </div>

                        {currentNews.isBreaking && (
                            <div className="p-4 bg-red-50 rounded-xl border border-red-100 animate-[fadeIn_0.3s_ease-out]">
                                <label className="block text-xs font-bold text-red-500 uppercase mb-1">Breaking Ticker Text (Short)</label>
                                <input type="text" value={currentNews.breakingText || ''} onChange={e => setCurrentNews({...currentNews, breakingText: e.target.value})} className="w-full p-2 border border-red-200 rounded-lg text-sm text-red-900 bg-white" placeholder="Text to scroll on ticker..." />
                            </div>
                        )}
                     </form>
                 </div>
                 <div className="p-4 border-t border-gray-100 bg-gray-50 flex justify-end gap-3">
                     {currentNews.id && (
                        <button type="button" onClick={() => { setIsEditingNews(false); handleDeleteNews(currentNews.id!); }} className="mr-auto text-red-500 hover:text-red-700 text-sm font-bold px-4 py-2 hover:bg-red-50 rounded-lg">Delete Entry</button>
                     )}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-[fadeIn_0.2s_ease-out]">
              <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg p-8">
                  ...
                  <div 
                      ...
                      className="border-2 border-dashed border-gray-300 rounded-xl p-10 flex flex-col items-center justify-center text-center hover:border-brand-cyan hover:bg-brand-cyan/5 transition-all cursor-pointer"
                  >
                      <input 
                          type="file" 
                          ref={bulkInputRef} 
                  ...
                  <div className="flex justify-end gap-3 mt-8">
                      <button onClick={() => setIsBulkModalOpen(false)} className="px-4 py-2 text-gray-500 hover:bg-gray-100 rounded-lg text-sm font-medium">Cancel</button>
                  </div>
              </div>
          </div>
      )}
